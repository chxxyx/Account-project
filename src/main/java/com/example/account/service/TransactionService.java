...
public class TransactionService {
    ...
    private final TransactionRepository transactionRepository; // 구현체
    private final AccountUserRepository accountUserRepository;
    private final AccountRepository accountRepository;

    /**
     * 정책 : 사용자가 없는 경우, 계좌가 없는 경우, 사용자 아이디 - 계정 소유주가 다른 경우,
     *      계좌가 이미 해지 상태인 경우, 거래 금액이 잔액보다 클 경우,
     *      거래 금액이 너무 작거나 너무 큰 경우 실패 응답
     */
    @Transactional
    public TransactionDto useBalance(Long userId, String accountNumber, Long amount){

       AccountUser user = accountUserRepository.findById(userId) // 사용자 조회
